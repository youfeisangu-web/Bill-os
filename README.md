...
```bash
# 1. クローン
git clone https://github.com/youfeisangu-web/Billia.git
cd Billia
...
│   │   ├── client.ts               # 取引先の管理
│   │   ├── payment.ts              # 入金データの保存
│   │   ├── payment-status.ts       # 月次入金台帳のステータス管理
│   │   ├── finance.ts              # 財務集計
...
### 音声入力で請求書作成
1. ユーザーが話す or テキスト入力
2. 音声認識でテキスト化（Web: Web Speech API / モバイル: Capacitor）
3. `actions/memo-parser.ts` がGemini APIに送信
4. AIが取引先・金額・件名・支払期日を抽出
5. 請求書エディタに自動入力

...
export default function DashboardSidebar() {
  ...
  return (
    <aside className="hidden w-64 flex-col border-r border-slate-200 bg-white px-4 py-6 lg:flex">
      <div className="px-3">
        <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Bill OS</p>
        <p className="mt-1 text-lg font-semibold text-slate-900">ダッシュボード</p>
      </div>

      <nav className="mt-8 flex flex-1 flex-col gap-2 text-sm text-slate-600">
        {navItems.map((item) => {
          const isActive =
            pathname === item.href ||

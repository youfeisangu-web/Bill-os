        ...
        <div className="flex items-center gap-2">
          ...
          {selectableQuotes.length > 0 && (
            <button
              ...
              disabled={isPending}
              className="inline-flex items-center rounded-xl bg-billio-green px-4 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-95 disabled:opacity-50"
            >
          ...
          <Link
            ...
            className="inline-flex items-center rounded-xl bg-billio-sidebar px-4 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-95"
          >
            見積書を作成
          </Link>
        </div>
          ...
          <tbody className="divide-y divide-billio-border-subtle">
            {quotes.length === 0 ? (
              ...
              quotes.map((quote) => {
                ...
                return (
                  <tr key={quote.id} className="text-billio-text-muted">
                    <td className="px-4 py-4">
                      {canSelect ? (
                        <input
                          ...
                          className="h-4 w-4 rounded border-stone-300 text-billio-green focus:ring-billio-green"
                        />
                      ) : null}
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={`font-medium ${
                          statusTone[quote.status] ?? "text-billio-text-muted"
                        }`}
                      >
                        {quote.status}
                      </span>

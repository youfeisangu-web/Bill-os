...
import { importDocument, type DocumentImportData } from "@/app/actions/ocr-document";
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          const result = await importDocument(formData, "quote");
      ...
      if (results.length > 0) {
        sessionStorage.setItem(QUOTE_OCR_STORAGE_KEY, JSON.stringify(results[0].data));
        // 複数ファイルがある場合は、残りをキューに保存
        if (results.length > 1) {
          sessionStorage.setItem(
            QUOTE_OCR_STORAGE_KEY + "_queue",
            JSON.stringify(results.slice(1))
          );
        }
        router.push("/dashboard/quotes/new?fromOcr=1");

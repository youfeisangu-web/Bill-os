    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-1">
          ...
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">請求書一覧</h1>
        </div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-50">請求書一覧</h2>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {invoices.length === 0 ? (
                ...
                invoices.map((invoice) => (
                  <tr key={invoice.id} className="text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                    <td className="px-4 py-4 font-medium text-blue-600 dark:text-blue-400">
                      <Link href={`/dashboard/invoices/${invoice.id}`}>
                        {invoice.id}
                      </Link>
                    </td>
                    <td className="px-4 py-4 dark:text-slate-400">{invoice.client?.name ?? "-"}</td>
                    <td className="px-4 py-4 dark:text-slate-400">{formatDate(invoice.issueDate)}</td>
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={`font-medium ${
                          statusTone[invoice.status] ?? "text-slate-600 dark:text-slate-400"
                        }`}
                      >
                        {invoice.status}
                      </span>
